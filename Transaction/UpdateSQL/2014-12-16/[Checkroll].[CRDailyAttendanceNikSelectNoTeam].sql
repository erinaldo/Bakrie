...
/****** Object:  StoredProcedure [Checkroll].[CRDailyAttendanceNikSelectNoTeam]    Script Date: 15/12/2014 13:27:35 ******/
DROP PROCEDURE [Checkroll].[CRDailyAttendanceNikSelectNoTeam]
GO
...
CREATE PROCEDURE [Checkroll].[CRDailyAttendanceNikSelectNoTeam]
@EmpCode nvarchar(50),
@EmpName nvarchar (50),
...
WHERE   GangEmployeeID is null and CREmployee.EstateID =@EstateID 
...
and GangEmployeeID is null and CREmployee.EstateID =@EstateID 
AND ( CREmployee .DOJ < = @AttDate ) 
 AND ((case when CREmployee.StatusDate IS NOT NULL then 1  end =1 AND CREmployee.StatusDate >  @AttDate    )
 or (case when CREmployee.StatusDate IS NULL then 1 end= 1 AND CREmployee.Status= 'Active' ))
ORDER BY Checkroll.CREmployee.Category ,Checkroll.CREmployee.EmpName
...
FROM      Checkroll.CREmployee FULL OUTER JOIN
          Checkroll.GangEmployeeSetup ON Checkroll.CREmployee.EmpID = Checkroll.GangEmployeeSetup.EmpID FULL OUTER JOIN
          Checkroll.RateSetup ON Checkroll.CREmployee.Category = Checkroll.RateSetup.Category
WHERE  (EmpCode LIKE  '%'+ @empcode	+ '%')   and GangEmployeeID is null and CREmployee.EstateID =@EstateID 
AND ( CREmployee .DOJ < = @AttDate ) 
 AND ((case when CREmployee.StatusDate IS NOT NULL then 1  end =1 AND CREmployee.StatusDate >  @AttDate    )
 or (case when CREmployee.StatusDate IS NULL then 1 end= 1 AND CREmployee.Status= 'Active' ))
ORDER BY Checkroll.CREmployee.Category ,Checkroll.CREmployee.EmpName
...
FROM      Checkroll.CREmployee FULL OUTER JOIN
          Checkroll.GangEmployeeSetup ON Checkroll.CREmployee.EmpID = Checkroll.GangEmployeeSetup.EmpID FULL OUTER JOIN
          Checkroll.RateSetup ON Checkroll.CREmployee.Category = Checkroll.RateSetup.Category
WHERE (EmpName LIKE  '%'+ @EmpName	+ '%') and  CREmployee.EmpJobDescriptionId =@Mandor  and GangEmployeeID is null
...
and GangEmployeeID is null and CREmployee.EstateID =@EstateID AND ( CREmployee .DOJ < = @AttDate ) 
 AND ((case when CREmployee.StatusDate IS NOT NULL then 1  end =1 AND CREmployee.StatusDate >  @AttDate    )
 or (case when CREmployee.StatusDate IS NULL then 1 end= 1 AND CREmployee.Status= 'Active' ))
ORDER BY Checkroll.CREmployee.Category ,Checkroll.CREmployee.EmpName
END
...
FROM      Checkroll.CREmployee FULL OUTER JOIN
          Checkroll.GangEmployeeSetup ON Checkroll.CREmployee.EmpID = Checkroll.GangEmployeeSetup.EmpID FULL OUTER JOIN
          Checkroll.RateSetup ON Checkroll.CREmployee.Category = Checkroll.RateSetup.Category
WHERE CREmployee.EmpJobDescriptionId =@Mandor and  GangEmployeeID is null and CREmployee.EstateID =@EstateID 
AND ( CREmployee .DOJ < = @AttDate ) 
 AND ((case when CREmployee.StatusDate IS NOT NULL then 1  end =1 AND CREmployee.StatusDate >  @AttDate    )
 or (case when CREmployee.StatusDate IS NULL then 1 end= 1 AND CREmployee.Status= 'Active' ))
ORDER BY Checkroll.CREmployee.Category ,Checkroll.CREmployee.EmpName
