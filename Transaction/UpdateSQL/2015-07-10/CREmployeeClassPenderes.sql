...
/****** Object:  StoredProcedure [Checkroll].[CREmployeeByWorktypePenderes]    Script Date: 6/21/2015 6:45:51 PM ******/
...
FROM            Checkroll.CREmployee Inner JOIN
                             (SELECT        NIK, SUM(Latex) AS Latex, SUM(CupLamp) AS CupLump, SUM(TreeLace) AS TreeLace, CONVERT(char(7), DateRubber, 20) AS daterubber
                               ...
                               WHERE        (CONVERT(char(7), DateRubber, 20) ='2015-07') --2014-10 example format
...
WHERE         (Checkroll.CREmployee.EstateID = @EstateID)
END
