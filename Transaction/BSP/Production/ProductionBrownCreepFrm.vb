...
Imports System.Linq
Public Class ProductionBrownCreepFrm
    ...
    Dim rm As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(ProductionBrownCreepFrm))

    Dim CurrentHeader As New ProductionBrownCreepPPT

    Private Sub ProductionBrownCreepFrm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub getObjectFromUI(objBrownCreepPPT)

        With objBrownCreepPPT
            ...
            Try
                .SkimLatex = Decimal.Parse(txtSkimLatex.Text.Trim())
            Catch ex As Exception
                Throw New Exception("Invalid Skim Latex")
            End Try

            Try
                .Washing = Decimal.Parse(txtWashing.Text.Trim())
            Catch ex As Exception
    ...
    Private Sub SaveAll(objBrownCreepPPT)
        Dim rowaffected As Integer = 0
        Dim intResult As Integer = 0
        Dim ds As New DataSet
        Try
            ProductionBrownCreepBOL.Save(objBrownCreepPPT)
    ...
    Sub ResetAllSaveContrls()
        ...
        txtSkimLatex.Text = String.Empty
    ...
    Private Sub GridViewBind()
        ResetAllSaveContrls()
        Dim objBOL As New ProductionBrownCreepBOL
        Dim dt As New ObservableCollection(Of ProductionBrownCreepPPT)

        Dim SearchObjPPT As ProductionBrownCreepPPT = New ProductionBrownCreepPPT
        ...
        Try
            dt = New ObservableCollection(Of ProductionBrownCreepPPT)(objBOL.GetSearchResults(SearchObjPPT))
        Catch ex As Exception
    ...
    Private Sub Add()
        btnAddFlag = True
        CurrentHeader = New ProductionBrownCreepPPT()
        ResetAllSaveContrls()
        ...
        If objUserLoginBOl.Privilege(btnSaveAll, AddToolStripMenuItem, EditToolStripMenuItem, DeleteToolStripMenuItem, PrivilegeError) Then
            If EditToolStripMenuItem.Enabled = True Then
                If dgvView.RowCount > 0 And dgvView.SelectedRows.Count > 0 Then
                    Dim currentSearchData As ObservableCollection(Of ProductionBrownCreepPPT) = (dgvView.DataSource)
                    ...
                    txtSkimLatex.Text = CurrentHeader.SkimLatex
                    txtWashing.Text = CurrentHeader.Washing
        ...
        If objUserLoginBOl.Privilege(btnSaveAll, AddToolStripMenuItem, EditToolStripMenuItem, DeleteToolStripMenuItem, PrivilegeError) Then
            If DeleteToolStripMenuItem.Enabled = True Then
                If dgvView.RowCount > 0 Then
                    ...
                    Try
                        ProductionBrownCreepBOL.DeleteLastRSS(CurrentHeader)
                        'DisplayInfoMessage("Msg04")
