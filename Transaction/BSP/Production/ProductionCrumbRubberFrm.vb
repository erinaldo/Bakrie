...
Imports System.Linq
Public Class ProductionCrumbRubberFrm
    ...
    Dim CurrentHeader As New ProductionCrumbRubberPPT

    Private Sub ProductionCrumbRubberFrm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub StockAwalFill()
        ...
        dt = objdb.ExecQuery("[Production].[ProductionCrumbRubberSIRStockAwal]", params).Tables(0)
        If dt.Rows.Count > 0 Then
            txtStAwal.Text = dt.Rows(0)(0).ToString()
        Else
            txtStAwal.Text = "0.00"
        End If
    End Sub
    ...
    Sub getObjectFromUI(objCrumbRubberSIRPPT)

        With objCrumbRubberSIRPPT
            ...
            Try
                .StockAwalDry = Decimal.Parse(txtStAwal.Text.Trim())
            Catch ex As Exception
                Throw New Exception("Invalid Stock Awail Dry")
            End Try

            Try
                .IntakeDry = Decimal.Parse(txtIntakeDry.Text.Trim())
            Catch ex As Exception
                Throw New Exception("Invalid Intake Dry")
            End Try
            ...
            Try
                .AdjustmentDry = Decimal.Parse(txtAdjust.Text.Trim())
            Catch ex As Exception
                Throw New Exception("Invalid Adjustment Dry")
            End Try

            Try
                .StockAkhirDry = Decimal.Parse(txtStkAkhir.Text.Trim())
            Catch ex As Exception
                Throw New Exception("Invalid Stock Akhir Dry")
            End Try
    ...
    Private Sub SaveAll(objCrumbRubberSIRPPT)
        Dim rowaffected As Integer = 0
        Dim intResult As Integer = 0
        Dim ds As New DataSet
        Try
            ProductionCrumbRubberBOL.Save(objCrumbRubberSIRPPT)
    ...
    Private Sub Add()
        btnAddFlag = True
        CurrentHeader = New ProductionCrumbRubberPPT()
        ResetAllSaveContrls()
    ...
    Sub ResetAllSaveContrls()
        ...
        txtStAwal.Text = String.Empty
        txtStkAkhir.Text = String.Empty
        txtIntakeDry.Text = String.Empty
        ...
        If objUserLoginBOl.Privilege(btnSaveAll, AddToolStripMenuItem, EditToolStripMenuItem, DeleteToolStripMenuItem, PrivilegeError) Then
            If EditToolStripMenuItem.Enabled = True Then
                If dgvView.RowCount > 0 And dgvView.SelectedRows.Count > 0 Then
                    Dim currentSearchData As ObservableCollection(Of ProductionCrumbRubberPPT) = (dgvView.DataSource)
                    ...
                    txtStAwal.Text = CurrentHeader.StockAwalDry
                    txtStkAkhir.Text = CurrentHeader.StockAkhirDry
                    txtIntakeDry.Text = CurrentHeader.IntakeDry
                    txtAdjust.Text = CurrentHeader.AdjustmentDry
        ...
        If objUserLoginBOl.Privilege(btnSaveAll, AddToolStripMenuItem, EditToolStripMenuItem, DeleteToolStripMenuItem, PrivilegeError) Then
            If DeleteToolStripMenuItem.Enabled = True Then
                If dgvView.RowCount > 0 Then
                    ...
                    Try
                        ProductionCrumbRubberBOL.DeleteLastCrumbRubber(CurrentHeader)
                        'DisplayInfoMessage("Msg04")
    ...
    Private Sub GridViewBind()
        ResetAllSaveContrls()
        Dim objBOL As New ProductionCrumbRubberBOL
        Dim dt As New ObservableCollection(Of ProductionCrumbRubberPPT)

        Dim SearchObjPPT As ProductionCrumbRubberPPT = New ProductionCrumbRubberPPT
        ...
        Try
            dt = New ObservableCollection(Of ProductionCrumbRubberPPT)(objBOL.GetSearchResults(SearchObjPPT))
        Catch ex As Exception
    ...
    Private Sub btnprod_Click(sender As System.Object, e As System.EventArgs) Handles btnprod.Click
        Dim params(1) As SqlParameter
        params(0) = New SqlParameter("@EstateID", GlobalPPT.strEstateID)
        params(1) = New SqlParameter("@Form", "SIR 3CV/L")
...
#End Region
    ...
    Private Sub txtRecDry_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtRecDry.TextChanged
        txtIntakeDry.Text = Val(txtStAwal.Text) + Val(txtRecDry.Text)
        txtStkAkhir.Text = Val(txtIntakeDry.Text) - Val(txtLatexProcess.Text)
    End Sub

    Private Sub txtLatexProcess_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtLatexProcess.TextChanged
        txtStkAkhir.Text = Val(txtIntakeDry.Text) - Val(txtLatexProcess.Text)
    End Sub
End Class